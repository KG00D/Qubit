...
const HoldingTransactions = () => {
  ...
  const accountTransactionsData = useSelector(
    (state) => state.transactions.accountTransactions
  );
  const transactionsData = accountTransactionsData
  ...
  const handleUpdate = (transactionId, updatedTransaction) => {
    dispatch(
      updateTransaction(accountId, holdingId, transactionId, updatedTransaction)
    )
      .then(() => {
        dispatch(fetchHoldingTransactions(accountId, holdingId));
      })
