...
const LandingPage = () => {
    ...
    class Particle {
        constructor(canvas) {
            this.canvas = canvas;
            this.reset();
            this.speedY = Math.random() > 0.5 ? (Math.random() * randoSpeed) * -1 : (Math.random() * randoSpeed);
            this.speedX = Math.random() > 0.5 ? (Math.random() * randoSpeed) * -1 : (Math.random() * randoSpeed);
        }
    ...
    useEffect(() => {
        const percentageTracker = () => {
            if (loading) {
                ...
                percentageRef.current.style.transform = `translateY(${top - window.innerHeight}px)`;
    ...
    return (
        <div>
            ...
            <div ref={splashScreenRef} className="splash__screen">
                ...
                <div ref={progressBarRef} className="progress__bar" style={{ height: `${progress}%` }}></div>
                <div ref={percentageRef} className="percentage">{progress}%</div>
            </div>
        </div>
    );
};
