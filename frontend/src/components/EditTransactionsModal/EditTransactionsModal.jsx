...
const EditTransactionsModal = ({
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const preparedTransactionData = Object.entries(updatedTransaction).reduce(
      (acc, [key, value]) => {
    ...
    if (isAdding || !preparedTransactionData.id) {
      ...
      dispatch(
        updateTransaction(preparedTransactionData.id, preparedTransactionData)
      );
    }
  ...
  const holdingsOptions =
    transactions.length > 0
      ? transactions.map((AccountTransactions) => (
          <option
            key={AccountTransactions.id}
            value={AccountTransactions.securityName}
          >
            {AccountTransactions.securityName}
          </option>
        ))
      : [
          <option key="no-holdings" value="" disabled>
      ...
      <div className="modal-content transactions-modal-content">
        ...
        <form onSubmit={handleSubmit} className="transactions-modal-form">
          ...
          <div className="transactions-modal-buttons">
            <button type="submit" className="transactions-modal-button submit">
