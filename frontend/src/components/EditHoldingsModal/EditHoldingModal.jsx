...
const EditHoldingModal = ({
  ...
  const [updatedHolding, setUpdatedHolding] = useState(
  ...
  useEffect(() => {
    if (!isAdding && holding) {
      setUpdatedHolding({
        ...holding,
        positionOpenDate: formatDateForInput(holding.positionOpenDate) || "",
      });
    } else {
  ...
  const handleAddNewHolding = (newHolding) => {
    const holdingExists = existingHoldings.some(
      (existingHolding) =>
    ...
    if (holdingExists) {
      alert(
        "This holding already exists in the account. Please add a transaction or remove the holding"
      );
      return;
    }

    dispatch(createHolding(accountId, newHolding)).finally(onClose);
  };

  const handleUpdate = () => {
    dispatch(updateHolding(accountId, holding.id, updatedHolding)).finally(
      onClose
    );
  };

  const handleDelete = () => {
    dispatch(deleteHolding(accountId, holding.id)).finally(onClose);
  };
        ...
        <form onSubmit={handleSubmit} className="modal-form">
          ...
          <div className="modal-buttons">
            <button type="submit" className="modal-button submit">
              {isAdding ? "Add Holding" : "Save Changes"}
            </button>
            {!isAdding && (
              <button
                type="button"
